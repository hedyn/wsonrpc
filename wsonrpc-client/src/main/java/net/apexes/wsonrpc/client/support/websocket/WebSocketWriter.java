...
class WebSocketWriter  {
    ...
    WebSocketWriter(WebSocketClient websocket, String threadBaseName, int clientId) {
        innerThread = WebSocketClient.getThreadFactory().newThread(new Runnable() {
            @Override
            public void run() {
                runWriter();
            }
        ...
        WebSocketClient.getIntializer().setName(getInnerThread(), threadBaseName + "Writer-" + clientId);
    ...
    synchronized void send(byte opcode, boolean masking, byte[] data) throws IOException {
        ...
        if (stop && (closeSent || opcode != WebSocketClient.OPCODE_CLOSE)) {
